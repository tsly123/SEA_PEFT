...
def get_loader(args):
    ...
    # Randomly select k support samples plus held-out validation batches
    random.seed(args.iFold - 1)
    selected = random.sample(data_dicts, min(args.k + 1, 30))

    # Split into train/validation (support = k, validation = 2 when available)
    num_val = 1
    data_dicts_train = selected[:-num_val]
    data_dicts_val = selected[-num_val:]
    ...
    if args.early_stop_criteria == "val":
        ...
        data_val_dict = {
            "image": torch.cat([sample["image"].unsqueeze(0) for sample in data_val], 0),
            "label": torch.cat([sample["label"].unsqueeze(0) for sample in data_val], 0),
            "name": [entry["name"] for entry in data_dicts_val],
        }
        args.data_val_dict = data_val_dict

    # Attach validation metadata (up to 2 unique samples) for on-demand batch generation
    args.validation_entries = data_dicts_val[:num_val]#[:2]
    args.validation_transform = transforms
