...
from fseft.config_loader import ConfigLoadError, load_candidates, select_candidate
from fseft.peft_configurator import apply_candidate_to_model
from fseft.utils import load_model, set_model_peft
from sa_peft.manifest import load_manifest
from sa_peft.units import UnitRegistry
...
def process(args):
    ...
    # Phase 3: override epochs if provided
    if args.epochs is not None:
        args.adapt_hp["epochs"] = args.epochs
    ...
    for iFold in args.folds:
        ...
        # Phase 3: Build unit registry from manifest so we can replay configs
        if args.manifest_path:
            manifest = load_manifest(args.manifest_path)
        else:
            manifest = load_manifest(None)
        registry = UnitRegistry(manifest, model)
        model.unit_registry = list(registry)

        # Phase 3: Apply exported configuration if provided
        if args.config_file:
            candidates = load_candidates(args.config_file)
            candidate = select_candidate(candidates, args.config_id)
            apply_candidate_to_model(model, candidate)
            print(
                f"Loaded config #{candidate.rank} '{candidate.label}' "
                f"(dice={candidate.dice:.4f}, units={candidate.unit_count})"
            )
    ...
    if args.path_results is not None:
        path_results = os.path.join(args.path_results, f"{args.organ}_incycle_results.json")
    else:
        path_results = args.out_path + args.organ + f'_results_rerun_mainfseft_earlystop{args.early_stop_criteria}.json'
...
def main():
    ...
    parser.add_argument('--dataset', default="flare", help='Options: totalseg - flare')
    parser.add_argument('--organ', default='stomach',
    ...
    parser.add_argument('--visualization', default=True, type=lambda x: (str(x).lower() == 'true'))
    parser.add_argument('--manifest_path', default=None, help='Optional manifest path for rebuilding PEFT registry')

    parser.add_argument("--config_file", default=None, help="Path to final_configs.yaml or .csv from SA-PEFT export")
    parser.add_argument("--config_id", type=int, default=1, help="Rank of configuration to apply (1 = best)")
    parser.add_argument("--epochs", type=int, default=None, help="Override training epochs (default 200)")
    parser.add_argument('--path_results', default=None, help='Optional manifest path for rebuilding PEFT registry')
    parser.add_argument("--lr", type=float, default=None, help="Rank of configuration to apply (1 = best)")
    parser.add_argument("--tsteps", type=int, default=None, help="Rank of configuration to apply (1 = best)")
